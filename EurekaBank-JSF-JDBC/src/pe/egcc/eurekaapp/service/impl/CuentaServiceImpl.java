...
public class CuentaServiceImpl implements CuentaServiceEspec {
	...
	public void procRetiro(String cuenta, double importe, String clave, String codEmp) {
		...
		try {
			...
			String sql = "select DEC_CUENSALDO, INT_CUENCONTMOV " + "from cuenta where CHR_CUENCODIGO = ? "
					+ "for update wait 5";
			...
			sql = "insert into movimiento(CHR_CUENCODIGO," + "INT_MOVINUMERO,DTT_MOVIFECHA,"
					+ "CHR_EMPLCODIGO,CHR_TIPOCODIGO," + "DEC_MOVIIMPORTE) " + "values(?, ?, SYSDATE, ?, '004', ?)";
			pstm = cn.prepareStatement(sql);
			pstm.setString(1, cuenta);
			pstm.setInt(2, cont);
	...
	public List<Map<String, ?>> getMovimientos(String cuenta) {
		...
		try {
			...
			String sql = "SELECT SUCUCODIGO, SUCUNOMBRE, CLIECODIGO, "
					...
					+ "CUENSALDO, CUENESTADO, MOVINUMERO, MOVIFECHA, " + "MOVIIMPORTE, CUENREFERENCIA, TIPOCODIGO, "
					+ "TIPONOMBRE, TIPOACCION, MONECODIGO, MONENOMBRE  " + "FROM V_MOVIMIENTO WHERE CUENCODIGO = ? ";
			PreparedStatement pstm = cn.prepareStatement(sql);
			pstm.setString(1, cuenta);
			ResultSet rs = pstm.executeQuery();
